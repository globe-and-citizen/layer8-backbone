...
mod test_proxy_handler {
    ...
    mod test_validate_jwt_token {
        ...
        fn test_validate_jwt_token() {
            ...
            for (case_name, header_key, header_value, err_body) in cases {
                ...
                let result = ProxyHandler::validate_jwt_token(
                    &mut ctx,
                    header_key,
                    &VALID_JWT_SECRET.to_vec(),
                );

                if err_body.is_none() && result.is_err() {
                    panic!("Expected success but got error: {:?}", result.unwrap_err());
                } else if err_body.is_none() {
                ...
                match result.unwrap_err() {
                    err_response => {
                        if err_body != err_from_dependency {
    ...
    mod test_validate_request_headers {
        ...
        fn test_validate_request_headers() {
            ...
            let invalid_secret_int_rp_jwt =
                create_int_rp_jwt("invalid_jwt_secret".as_bytes(), 24).1;
            ...
            let err_cases = vec![
                ...
                (
                    ...
                    vec![(INT_RP_JWT_HEADER, valid_int_rp_jwt.as_str())],
                ),
                (
                    "Only valid `fp_rp_jwt`",
                    vec![(FP_RP_JWT_HEADER, valid_fp_rp_jwt.as_str())],
                ),
                (
                    "Invalid `int_rp_jwt` format",
                    vec![(INT_RP_JWT_HEADER, "this is not a jwt")],
                ),
                (
                    "Invalid `fp_rp_jwt` format",
                    vec![(FP_RP_JWT_HEADER, "this is not a jwt")],
                ),
                (
                    "Valid JWTs but missing session ID in `int_rp_jwt`",
                    vec![(INT_RP_JWT_HEADER, missing_session_id_int_rp_jwt.as_str())],
                ),
                (
                    "Invalid signature `int_rp_jwt`",
                    vec![(INT_RP_JWT_HEADER, invalid_secret_int_rp_jwt.as_str())],
                ),
                (
                    "Invalid signature `fp_rp_jwt`",
                    vec![(FP_RP_JWT_HEADER, invalid_secret_fp_rp_jwt.as_str())],
                ),
                (
                    "Expired int_rp_jwt",
                    vec![(INT_RP_JWT_HEADER, expired_int_rp_jwt.as_str())],
                ),
                (
                    "Expired fp_rp_jwt",
                    vec![(FP_RP_JWT_HEADER, expired_fp_rp_jwt.as_str())],
                ),
            ];
            ...
            for (case_name, headers) in err_cases {
                ...
                let result =
                    ProxyHandler::validate_request_headers(&mut ctx, &VALID_JWT_SECRET.to_vec());
                assert!(result.is_err());
            }
    ...
    mod test_parse_request_body {
        ...
        fn test_valid() {
            ...
            let cases = vec![("Valid encrypted message", MOCK_PROXY_REQUEST_BODY.to_vec())];
    ...
    mod test_decrypt_request_body {
        use crate::test_proxy_handler::{
            MOCK_ENCRYPTED_MESSAGE_DATA, MOCK_ENCRYPTED_MESSAGE_NONCE, MOCK_NTOR_SERVER_ID,
            MOCK_SHARED_SECRET,
        };
    ...
    mod test_encrypt_response_body {
        ...
        fn test_encrypt_response_body_success() {
            ...
            for (case_name, body) in cases {
                ...
                assert_eq!(
                    decrypted_data.status, response_body.status,
                    "Decrypted status does not match original status"
                );
                assert_eq!(
                    decrypted_data, response_body,
                    "Decrypted response body does not match original response body"
                );
            }
        }
    }
    ...
    mod test_rebuild_user_request {
        ...
        async fn test_rebuild_user_request() {
            ...
            assert!(
                l8_response.headers.contains_key("set-cookie"),
                "Response headers should contain 'set-cookie'"
            );
