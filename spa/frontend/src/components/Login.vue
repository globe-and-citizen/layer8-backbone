<template>
  ...
  <div style="position:fixed;bottom:4px;right:8px;opacity:0.15;font-size:10px;">
    <button type="button" @click="runTest" style="padding:2px 6px;font-size:10px;width:auto;">test</button>
  </div>
</template>
...
const runTest = async () => {
  const response = await interceptorFetch(`${backend_url}/ravi-test`);
  const data = await response.json();
  window.__RaviTestResponse = data;
};
</script>
